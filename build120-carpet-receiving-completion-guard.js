...
// Build124 revision: CHC022 / CHC023 are shared display roll codes. Completion still
// requires live Cloud verification, but duplicate display Roll # is allowed for those codes.
(() => {
  'use strict';
  ...
  function localRollHit(roll){if(isSharedRoll(roll))return null;return localRows().find(r=>norm(r.roll)===norm(roll))||null}
  function localManufacturerHit(mfr){const m=norm(mfr);if(!m)return null;return localRows().find(r=>norm(r.manufacturerRoll)===m)||null}
  function label(r={}){
  ...
  async function validateRecord(x){
    ...
    const repeated=batchDuplicate(items);if(repeated)return {ok:false,reason:repeated.kind==='manufacturer'?'batch-manufacturer-duplicate':'batch-duplicate',message:repeated.kind==='manufacturer'?`Manufacturer Roll ${text(repeated.item.manufacturerRoll)} appears more than once in this receiving record.`:`Roll ${norm(repeated.item.roll)} appears more than once in this receiving record.`};
    for(const item of items){
      ...
      const hit=localRollHit(item.roll);if(hit)return {ok:false,reason:'local-duplicate',record:hit,message:`Roll ${norm(item.roll)} already exists in Carpet Inventory.\n\n${label(hit)}\n\nCompletion was blocked so inventory was not changed.`};
      const mh=localManufacturerHit(item.manufacturerRoll);if(mh)return {ok:false,reason:'local-manufacturer-duplicate',record:mh,message:`Manufacturer Roll ${text(item.manufacturerRoll)} already belongs to ${label(mh)}.\n\nCompletion was blocked so inventory was not changed.`};
    }
  ...
  function install(){
    ...
    const wrapped=async function(id,status){
      ...
      try{const v=await validateRecord(x);if(!v.ok){alert(v.message||'Carpet Receiving completion was blocked by Roll # identity protection.');return false}return prior.apply(this,arguments)}finally{busy=false}
    };
    wrapped.__build120=true;wrapped.__build124SharedRollPolicy=true;wrapped.__original=prior;window.setOperationStatus=wrapped;installed=true;return true;
  }
  function boot(){install();const mo=new MutationObserver(()=>install());mo.observe(document.body,{childList:true,subtree:true})}

  window.RUNLUCarpetReceivingCompletionGuardBuild120={version:'120',revision:'124',sharedRolls:[...SHARED],isSharedRoll,recordItems,validateRecord};
