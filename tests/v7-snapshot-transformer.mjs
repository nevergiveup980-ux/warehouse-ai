...
assert.deepEqual(m.summary.derived_carpet_rolls,{valid:2});
...
const strong=m.derived_carpet_rolls.find(x=>x.record_id==='CARPET_PHYSICAL:source_mfg:CHC1|100');
assert.ok(strong);
assert.equal(strong.reason,'CARPET_PHYSICAL_REPLAY_GROUP_READY');
assert.equal(strong.transformed.roll_number,'CHC1');
assert.deepEqual(strong.source_payload.member_record_ids,['C1','C2']);
const alias=m.derived_carpet_rolls.find(x=>x.record_id==='CARPET_ALIAS:LEGACY-900');
assert.ok(alias);
