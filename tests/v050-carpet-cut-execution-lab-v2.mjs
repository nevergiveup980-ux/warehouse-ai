...
// Extract the exact production function body. Important: some Warehouse functions use
// default object parameters such as item={}, so the first "{" after the function name
// is NOT necessarily the function body. Find the closing parameter parenthesis first.
function extractFunction(name){
  const needle=`function ${name}`;
  const start=source.indexOf(needle);if(start<0)throw new Error(`Production function not found: ${name}`);
  const openParen=source.indexOf('(',start);if(openParen<0)throw new Error(`Opening parenthesis not found: ${name}`);
  let parenDepth=0,quote='',escape=false,lineComment=false,blockComment=false,closeParen=-1;
  for(let i=openParen;i<source.length;i++){
    ...
    if(c==='(')parenDepth++;else if(c===')'){parenDepth--;if(parenDepth===0){closeParen=i;break}}
  }
  if(closeParen<0)throw new Error(`Closing parenthesis not found: ${name}`);
  const brace=source.indexOf('{',closeParen+1);if(brace<0)throw new Error(`Function body not found: ${name}`);
