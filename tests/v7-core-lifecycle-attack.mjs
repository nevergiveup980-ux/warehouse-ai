...
const files=['receiving','transfer','cut','shipping','return','supplier-return'].map(x=>fs.readFileSync('v7/'+x+'-engine-1.0.sql','utf8'));
...
assert.ok((all.match(/insert into warehouse_v7\.event/g)||[]).length===8);
assert.ok((all.match(/insert into warehouse_v7\.inventory_movement/g)||[]).length===8);
