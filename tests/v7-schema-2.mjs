...
assert.match(s,/primary key\(tenant_id,id\)/); assert.match(s,/payload_fingerprint text not null/);
assert.match(s,/original_sixteenths bigint/); assert.match(s,/remaining_sixteenths bigint/);
assert.match(s,/unique\(tenant_id,roll_number\)/); assert.ok((s.match(/on delete restrict/g)||[]).length>=8);
