...
r=run(f"select warehouse_v7.stage_legacy_record('{T}','runlu_product_master_v21','PRD-IMM','{{\"name\":\"Immutable\",\"base_unit\":\"BOX\"}}'::jsonb);")
...
state=run(f"select source_dataset||'|'||source_record_id||'|'||classification||'|'||(source_payload->>'name') from warehouse_v7.migration_staging where tenant_id='{T}' and id='{sid}';")
assert value(state)=='runlu_product_master_v21|PRD-IMM|valid|Immutable',state.stdout
