...
insert into warehouse_v7.product(tenant_id,id,name,base_unit,lifecycle) values('{T}','{P}','Attack Product','BOX','active');
insert into warehouse_v7.location(tenant_id,id,code,lifecycle) values('{T}','{L}','A1','active');
...
def q(c): return f"""set request.jwt.claim.sub='{A}'; select warehouse_v7.ship_stock('{T}','{c}','{S}',1,7,'{{"attack":"same-version"}}','{A}','DEV');"""
