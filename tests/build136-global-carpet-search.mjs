...
const source=fs.readFileSync(new URL('../build136-global-carpet-search.js',import.meta.url),'utf8');
assert.match(source,/Global search · all roll statuses and racks/,'search result must explain global scope');
assert.match(source,/carpetQuickFilter=''|quick:''/,'global search must bypass quick/status filters');
assert.match(source,/carpetRackFilter=''|rack:''/,'global search must bypass rack filters');
...
vm.runInContext(source,context,{filename:'build136-global-carpet-search.js'});
assert.equal(context.renderCarpetInventory.__build136,true,'renderCarpetInventory must be wrapped');
...
console.log('Build136 global carpet search: PASS');
