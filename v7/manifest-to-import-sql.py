...
def emit(item,derived=False):
    ...
    lines.append(f"select warehouse_v7.classify_legacy_record({q(tenant)}::uuid,{sid},{q(item['classification'])},{q(item.get('reason',''))},{q(actor)}::uuid) where exists (select 1 from warehouse_v7.migration_staging where tenant_id={q(tenant)}::uuid and id={sid} and classification<>'imported');")
