...
begin
 ...
 select * into s from warehouse_v7.stock_item
 where tenant_id=p_tenant and id=p_stock_item for update;
 stock_existed:=found;

 if stock_existed then
   ...
   update warehouse_v7.stock_item set quantity=quantity+p_quantity,version=new_version
   ...
   insert into warehouse_v7.stock_item(tenant_id,id,product_id,location_id,quantity,unit,version)
    values(p_tenant,p_stock_item,p_product,p_location,p_quantity,p_unit,new_version);
 end if;
 ...
 values(p_tenant,p_command,'stock_item',p_stock_item,'RECEIVED',new_version,
 ...
 out_result:=jsonb_build_object('status','committed','stock_item_id',p_stock_item,'received',p_quantity,
   'unit',p_unit,'new_version',new_version);
